...
export class AI{
    ...
    async retrieveVectorSearchResults(input: ChainState): Promise<ChainState> {
        ...
        const filteredResults = vectorResults.filter(([_, score]) => score > 0.5)

        this.params.debugLog(`\n📄 Chunks utilizados (score > 0.5): ${filteredResults.length} de ${vectorResults.length}`)
        filteredResults.forEach(([doc, score], idx) => {
            this.params.debugLog(`\n  ── Chunk ${idx + 1} (score: ${score.toFixed(4)}) ──`)
            this.params.debugLog(`  Fonte: ${doc.metadata?.source ?? 'desconhecida'} | Página: ${doc.metadata?.page ?? 'N/A'}`)
            this.params.debugLog(`  Conteúdo:\n  ${doc.pageContent.replace(/\n/g, '\n  ')}`)
        })
        this.params.debugLog('')
